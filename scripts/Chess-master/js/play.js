...
//点击棋盘事件
play.msg_to_move = function (Point){
	if (!play.isPlay) return false;
	//add by cjr

	// play.moveByNotation("炮2进3");
	pace = Point[0]
...
play.msg_to_move2 = function (Point){
	if (!play.isPlay) return false;
	//add by cjr
...
//Ai自动走棋
play.UserAutoPlaybakok = function (){
	//return

	play.my = -1 ;
	var pace=Human_AI.init(play.pace.join(""))
	alert(pace);
...
play.UserAutoPlay = function (player_side){
	//return
...
//cjr add
play.moveByNotation = function(notation) {
    var pieces = {
        "车": "c", "马": "m", "相": "x", "象": "x", "士": "s", "仕": "s",
        "将": "j", "帅": "j", "炮": "p", "兵": "z", "卒": "z"
    };

    // 棋谱术语解析
    var piece = notation[0];
    var from = notation[1];
    var to = notation[3];

    // 查找棋子对象
    var key = null;
    for (var k in play.mans) {
        if (play.mans[k].text === piece) {
            key = k;
            break;
        }
    }
    ...
    // 计算目标位置
    var fromX = parseInt(from) - 1;
    var fromY = man.y;
    var toX = parseInt(to) - 1;
    var toY = man.y;
    ...
    if (man.my === 1) { // 红方
        if (notation[2] === '平') {
            movePiece(key, fromX, fromY, toX, toY);
        } else if (notation[2] === '进') {
            toY = fromY - parseInt(to) + 1;
            movePiece(key, fromX, fromY, fromX, toY);
        } else if (notation[2] === '退') {
            toY = fromY + parseInt(to) - 1;
            movePiece(key, fromX, fromY, fromX, toY);
        }
    } else { // 黑方
...
function movePiece(key, fromX, fromY, toX, toY) {
