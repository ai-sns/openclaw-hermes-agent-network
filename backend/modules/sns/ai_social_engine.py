...
from PyQt6.QtWidgets import QWidget, QDialog, QScrollArea, QGridLayout, QPushButton, QVBoxLayout, QFileDialog, QMessageBox
...
import random
from map_task_manager import MapTaskManager
from js_task_manager import JsTaskManager
from PyQt6.QtWidgets import QTextEdit
from PyQt6.QtCore import Qt
from prompts_sns import  PromptManager
from PyQt6.QtGui import QTextCursor
from aimapcfgdialog import ConfigDialog as AiMapCfgDialog
from jobselect import ProfessionDialog
from runtoolsetting import EventProgramDialog
# 配置日志记录
...
class WorkerThread(QThread):
    ...
    def run(self):
        ...
        if self.type_flag == "command":
            ...
            tool_id,tool_name, what_to_do = question.split("__AISNS_INT_SEPARATOR__")
            answer = agent.run_tool(tool_id,tool_name,what_to_do, self.messages, browser_page, self.task_id)

        if self.type_flag == "command" or self.type_flag == "tool":
...
class MessageBox(QWidget, Ui_MessageWidget):
    ...
    def __init__(self, application, con, ai_chat_cfg):
        super(MessageBox, self).__init__(application.map_window_widget)
        ...
        self.startButton.clicked.connect(self.toggle_status)
        self.humantakeoverCheckBox.clicked.connect(self.humantakeoverhandle)
        self.pauseCheckBox.clicked.connect(self.toggle_pause_task)
        ...
        self.pauseCheckBox.setChecked(self.pause_flag)
        self.messages = []
    ...
    def showCompletionDialogbakok(self):
        """显示对话框，供用户选择内容"""
        if self.current_talk_people:
            choices = ["My_Agent", self.current_talk_people["nick_name"]]  # 选择项列表
        else:
            choices = ["My_Agent"]
    ...
    def showCompletionDialog(self):
        ...
        # 初始化选项列表
        # 获取所有预设消息记录
        all_records = query_map_preset_msg_all()
        ...
        # 连接列表交互事件
        list_widget.itemDoubleClicked.connect(lambda item: self.insertCompletion(item.text(), dialog))
        list_widget.keyPressEvent = lambda event: self.handleKeyPress(event, list_widget, dialog)
    ...
    def _removeSelectedItem(self, list_widget):
        ...
        if selected_item:
            ...
            if reply == QtWidgets.QMessageBox.Yes:
                ...
                delete_map_preset_msg(selected_item.text())
            ...
            if self.human_take_over:
                ...
                if self.talk_type_combo.currentData()=="Mine":
                    if self.agent_replying_flag:
                        QMessageBox.information(self, "提示", "Agent正在完成上一个任务，请稍等...")
                        return
    ...
    def sendfile(self):
        ...
        parser.add_argument("-j", "--jid", dest="jid", default=self.ai_chat_cfg.account,
                            help="JID to use")
        parser.add_argument("-p", "--password", dest="password", default=self.ai_chat_cfg.password,
                            help="password to use")

        # Other options.
        parser.add_argument("-r", "--recipient", dest="recipient", required=False, default=self.current_talk_people["account"],
                            help="Recipient JID")
        ...
        xmpp = HttpUpload(
            jid=self.ai_chat_cfg.account,
            password=self.ai_chat_cfg.password,
            recipient=JID(self.current_talk_people["account"]),
    ...
    def send_file_bg(self, filepath, to_jid):
        if filepath == "":
            return ("")
        filename = Path(filepath).name
        if sys.platform == 'win32':
        ...
        # JID and password options.
        parser.add_argument("-j", "--jid", dest="jid", default="example@xabber.de",
                            help="JID to use")
        parser.add_argument("-p", "--password", dest="password", default="example",
                            help="password to use")

        # Other options.
        parser.add_argument("-r", "--recipient", dest="recipient", required=False, default="to_example@xabber.de",
                            help="Recipient JID")
        parser.add_argument("-f", "--file", dest="file", required=False, default=filepath,
                            help="File to send")
        parser.add_argument("--domain",
    ...
    def exitscene(self):
        self.map_mode = 'org'
        # self.load_url("http://localhost:8900/scripts/3d/girlmovementv15.html",False)
        self.load_url("http://localhost:8900/scripts/googlemap3d.html", False)
    ...
    def handle_open_sns_profile(self,url):
        # if not self.tab_profile:
        ...
        self.tabWidget.setCurrentWidget(self.tab_profile)
    ...
    def handle_show_game_target(self):
        reply = QtWidgets.QMessageBox.information(
            self,
            "任务说明",
            f"当前的任务和目标：在社交游戏中生存下去，并不断增加财富。\n您可在角色调教中进行任务目标调整。是否查看更多帮助?",
            QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.No,
            QtWidgets.QMessageBox.No
        )
    ...
    def handle_mapcfg_setting(self):
        record = query_AiChatCfg_map()
        dialog = AiMapCfgDialog(self.application,record)
        if dialog.exec_() == QDialog.Accepted:
            map_type = dialog.map_selected
            if map_type == "google":
                self.load_url("http://localhost:8900/scripts/googlemap3d.html", False)
            else:
                self.load_url("http://localhost:8900/scripts/map.html", False)
    ...
    def open_run_tool_setting(self):
        dialog = EventProgramDialog()
        dialog.exec()
        self.messageBrowser.setFocus()
    ...
    def toggle_output_checkbox(self, state):
        if state == Qt.CheckState.Checked.value:
            ...
            # print("self.splitter.width():",self.splitter.width())
            # if self.splitter.width()< 280:
            #     self.splitter.setSizes([1, 280])
            # else:
            #     self.splitter.setSizes([self.splitter.width() - 280, 280])  # 动态计算剩余空间
            self.splitter.setSizes([1, 280])
            # 设置第一个窗口可伸缩（拉伸因子为1），第二个窗口固定（拉伸因子为0）
            self.splitter.setStretchFactor(0, 1)  # 索引0（第一个窗口）可伸缩
            self.splitter.setStretchFactor(1, 0)  # 索引1（第二个窗口）固定
    ...
    def change_map(self):
        if self.map_type_combo.currentData() == "Google":
            self.load_url("http://localhost:8900/scripts/googlemap3d.html", False)
        else:
            self.load_url("http://localhost:8900/scripts/map.html", False)
...
"""
        ...
        self.thread = WorkerThread(agent, question, messages, self.messageBrowser, self.conversation_id, None, type_flag)
        self.thread.ask_command_finished.connect(self.on_agent_return_instruction)
        self.thread.start()
        self.agent_replying_flag = True
        self.loading_tab.start_loading()
...
{content}
    ...
    def bak_write_thinking_process_to_pane_bak(self, content, function_name=""):
        # 假设 self.thinking_edit 是 QTextEdit 的实例
        self.thinking_step_index += 1

        # 组合新内容
        new_content = f"\n🔶【{self.thinking_step_index}】{content}\n"
        if function_name:
            new_content += f" *function:* {function_name}\n"


        cursor = self.thinking_edit.textCursor()
        cursor.movePosition(QTextCursor.MoveOperation.Start)
        cursor.movePosition(QTextCursor.MoveOperation.NextBlock)
        # Insert two lines of text at the second block
        cursor.insertText(new_content)

        # 将光标移动到顶部
        cursor.movePosition(QTextCursor.MoveOperation.Start)  # 移动到开头
        self.thinking_edit.setTextCursor(cursor)  # 更新文本框的光标位置
    ...
    def insert_line_above_ongoing(self,text_edit: QTextEdit, new_line: str):
        """
            在文本中的📜【Process history】和⏳【On going】标记之间插入日志记录

            参数:
                original_text (str): 原始文本内容
                log_entry (str): 要插入的日志条目(不需要包含换行符或标记)

            返回:
                str: 插入日志后的完整文本
            """
        # 定义起始和结束标记
        start_marker = "📜【Process history】"
        end_marker = "⏳【On going】"
        original_text = text_edit.toPlainText()

        # 分割文本
        before_history = original_text.split(start_marker)[0]
        history_section = original_text.split(start_marker)[1].split(end_marker)[0]
        history_section = history_section.strip()
        after_ongoing = original_text.split(end_marker)[1]

        # 格式化新日志条目(添加序号)
        formatted_new_entry = f"\n\n{new_line}"
        ...
        # 重建文本内容
        updated_text = (
            f"{before_history}{start_marker}"
            f"{history_section}{formatted_new_entry}"
            f"{end_marker}{after_ongoing}"
        )
...
子任务列表，当前目标,当前目标和主任务相关性及评分，请按如下json格式输出:
{
    ...
    # 3.1构建完整的请求任务指示的提示词
    def compose_full_ask_contentbak(self, task_description, ability_list, question_to_llm):
        prompt = get_prompt_by_title("__instruction_to_process_activity_content__")
...
- 八达岭长城:著名旅游景点。坐标[116.0204,40.3606],距离60公里。\n
    ...
    def ask_human_to_check_skill(self):
        self.command_status = "wait_human_check_skill"
        # 创建提醒消息框
        msg_box = QMessageBox(self)
        msg_box.setIcon(QMessageBox.Icon.Information)
        msg_box.setText("您获得了新的技能，请去确认技能是否能用！")
        msg_box.setWindowTitle("技能提醒")
...
{lt(f"Talk to a people with {user_name} acount:{account},nationid:{nationid},content:{content}", f"和别人交谈 with {user_name} acount:{account},nationid:{nationid},content:{content}")}
    ...
    def end_task(self):
        record_id = self.taskmng.current_task_record.id
        self.stop_task()
        taskList = self.application.maptasklist
        item = self.find_item_by_id(taskList, record_id)
        item_icon = QIcon()
        item.setIcon(0, item_icon)
        update_map_task(record_id, status=2)
