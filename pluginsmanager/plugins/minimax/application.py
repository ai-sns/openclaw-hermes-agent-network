...
class Connector_LLM_Jiuzhou_yfd_Chatglm_Plugin(PluginCore):
    ...
    def __init__(self, logger: Logger) -> None:
        ...
        self.connection_mode = "OpenAI-customize-v2"
...
class LLMConnection():
    def __init__(self, url="",api_key="",model="", top_p=0.8, temperature=0.8, stream=True,max_tokens=256):
        self.url = url
    ...
    def convert_parameters_to_string(self,data):
        if isinstance(data, dict):
            if 'parameters' in data and isinstance(data['parameters'], dict):
                data['parameters'] = str(data['parameters'])
            for key, value in data.items():
                self.convert_parameters_to_string(value)
        elif isinstance(data, list):
            for item in data:
                self.convert_parameters_to_string(item)
    ...
    def generate_stream(
        ...
        self.convert_parameters_to_string(param)
        payload = json.dumps({
            "model": self.model,
            "messages": param.get("messages",None),
            "tools": param.get("tools", None),
            "tool_choice": "auto",
            "stream": True,
            "max_tokens": 1000,
