...
Do not apologize for making illegal moves.
    ...
    def handle_replying_a(self, *args, **kwagrs):
            parent = args[0]
            message = args[1]
            word = message
            if parent.__class__.__name__ == "TaskPage":
                ...
                if word != "__end_speak__":
                    ...
                    if k % 20 == 0:
                        result=self.get_text_before_last_punctuation(words)
                        self.queue.append(result[0].replace("#","").replace("-","").replace("*",""))
                        if not self.speech_start_flag:
                            self.speech_thread.start()
                            self.speech_start_flag=True
                        self.words = result[1]
    ...
    def get_text_before_last_punctuation(self,sentence):
        """
        Returns the substring of the given sentence that appears before
        the last punctuation mark, newline, or space.

        Parameters:
        sentence (str): The input sentence from which to extract the substring.

        Returns:
        str: The substring before the last punctuation, newline, or space.
        """

        # Define a regular expression pattern to match the last occurrence of punctuation, newline, or space
        pattern = r'[^\w\s]'
        ...
        # If no punctuation was found, return the entire sentence
        if last_pos is None:
            return sentence

        # Return the substring before the last punctuation
        return (sentence[:last_pos + 1],sentence[last_pos + 1:])
