...
from db.DBFactory import delete_AgentCfg,update_AgentCfg_by_user_id,query_AgentCfg
...
class FreezeTableDialog(QDialog):
    ...
    def deleteSelectedRows(self):
        ...
        if selected_indexes:
            ...
            if confirmation == QMessageBox.Yes:
                ...
                delete_AgentCfg(agent_user_id)
                del global_agent_list[agent_user_id]

                tool_box_item = self.app.toolBox_AgentChat.findChild(QWidget, agent_user_id)
    ...
    def update_agent_index(self):
        for row in range(self.model.rowCount()):
            ...
            update_AgentCfg_by_user_id(agent_id,position=row)
    ...
    def on_item_changed(self, item):
        if item.isCheckable():
            ...
            if item.checkState()== Qt.Unchecked:
                agent_user_id = self.model.index(item.row(), 1).data()
                item_to_move = self.app.toolBox_AgentChat.findChild(QWidget, agent_user_id)
                current_index = self.app.toolBox_AgentChat.indexOf(item_to_move)  # 获取当前索引
                ...
                del item_to_move
                update_AgentCfg_by_user_id(agent_user_id, is_show=False)
            else:
                agent_user_id = self.model.index(item.row(), 1).data()
                agent =  global_agent_list[agent_user_id]
                agent.agent_cfg.is_show=True
                update_AgentCfg_by_user_id(agent_user_id, is_show=True)
                pos = self.get_checked_count()
                self.app.createToolBoxUnit_AgentChat(agent, pos)
