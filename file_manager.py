...
class FileManager(QWidget):
    ...
    def showEvent(self, event):
        """窗口显示事件"""
        self.file_table.clearContents()  # 清空所有单元格的内容
        self.file_table.setRowCount(0)  # 设置行数为 0，删除所有行
    ...
    def show_context_menu(self, pos):
        ...
        open_action.triggered.connect(self.open_file_btn)
    ...
    def rename_file(self):
        ...
        selected_items = self.file_table.selectedItems()
        if not selected_items:
            QMessageBox.warning(self, "警告", "请先选择一个文件进行重命名。")
            return
        ...
        new_name, ok = QInputDialog.getText(self, "重命名文件", "新文件名：", text=old_name)
        if ok and new_name:
            new_file_path = os.path.join(dir_name, new_name)
            new_file_ext = os.path.splitext(new_file_path)[1]
            try:
                os.rename(file_path, new_file_path)
                self.file_table.setItem(row, 0, QTableWidgetItem(new_name))
                self.file_table.setItem(row, 2, QTableWidgetItem(new_file_ext))
                self.file_table.setItem(row, 4, QTableWidgetItem(new_file_path))
                QMessageBox.information(self, "重命名", f"文件已成功重命名为 '{new_name}'。")
            except Exception as e:
                QMessageBox.critical(self, "错误", f"重命名文件时发生错误: {e}")
