...
from db.DBFactory import delete_AgentCfg,update_prompt_frequent,query_AgentCfg,delete_prompt_frequent
...
class FreezeTableDialog(QDialog):
    ...
    def deleteSelectedRows(self):
        ...
        if selected_indexes:
            ...
            if confirmation == QMessageBox.Yes:
                delete_prompt_frequent(record_id)
                ...
                self.promptmng.taskpage.update_prompts_in_combobox()
    ...
    def update_prompt_index(self):
        for row in range(self.model.rowCount()):
            agent_id = self.model.index(row, 1).data()
            update_prompt_frequent(agent_id,position=row)
