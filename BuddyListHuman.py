...
class BuddyListHuman(QTreeWidget):
    """BuddyListHuman implements the view in a Tree of the Roster"""
    rename_signal = pyqtSignal(object)
    def __init__(self, parent):

        super(BuddyListHuman, self).__init__(parent)
        print("BuddyListHuman parent",parent)
    ...
    def setConnection(self, con):
        self.connection = con
        #一个BuddyListHuman对应一个用户的ConnectThread
    ...
    def addItem(self, jid):
        if self.connection:
            ...
            if jid not in self.buddies.keys():
                self.buddies[jid] = BuddyItemHuman(self.groups[group], jid, self.connection,self.mainwindow)
    ...
    def addGroup(self, group):
        if group:
            if group not in self.groups.keys():
                self.groups[group] = BuddyGroupHuman(group)
                self.tree[group] = {}
    ...
    def message(self, event):
        ...
        if buddy not in self.buddies.keys():
            self.buddies[buddy] = BuddyItemHuman(None, buddy)
