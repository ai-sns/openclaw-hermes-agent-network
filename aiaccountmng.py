...
class FreezeTableDialog(QDialog):
    ...
    def deleteSelectedRows(self):
        ...
        if selected_indexes:
            ...
            if confirmation == QMessageBox.Yes:
                ...
                delete_AiChatCfg(agent_user_id)


                tool_box_item = self.app.toolBox_AiChat.findChild(QWidget, agent_user_id)
                ...
                item_index = self.app.toolBox_AiChat.indexOf(tool_box_item)

                # 如果找到了该item，则进行删除
                if item_index != -1:
                    self.app.toolBox_AiChat.removeItem(item_index)
    ...
    def moveSelectedRowsUp(self):
        ...
        if selected_indexes:
            ...
            item_to_move = self.app.toolBox_AiChat.findChild(QWidget, agent_user_id)


            if item_to_move:
                current_index = self.app.toolBox_AiChat.indexOf(item_to_move)  # 获取当前索引
                item_icon = self.app.toolBox_AiChat.itemIcon(current_index)
                if current_index > 0:  # 确保不是第一个项
                    # 移动项的逻辑
                    self.app.toolBox_AiChat.removeItem(current_index)  # 移除当前项
                    self.app.toolBox_AiChat.insertItem(current_index - 1, item_to_move,item_icon, f"{name}")  # 将项插入到前面的位置
    ...
    def update_agent_index(self):
        for row in range(self.model.rowCount()):
            ...
            update_AiChatCfg_by_user_id(agent_id,position=row)
