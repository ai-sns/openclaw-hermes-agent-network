...
class WorkerThread(QThread):
    ...
    def __init__(self, agent, task_id, is_first, question, messages, llm_full_name,system_role_id,system_role_prompt, vector_path, embedding_model_name, modelname, web_browser, speaker, attachment_content_list, plugin_tool_record_selected_list, parent=None):
...
class TaskPage(QWidget, Ui_TaskPageWidget):
    ...
    def sendMessage(self):
        # self.message_handler.pass_message("cjrokfrompython")
        ...
        if self.messageEdit.toPlainText():
            ...
            if self.messageEdit.toPlainText() == "ymcymc":
                ...
                if "skill_id:" in text:
                    ...
                    match = re.search(r'skill_id:(.*)', input_string)
    ...
    def open_llm_cfg_dialog(self):
        ...
        model = QStandardItemModel()
        header = ["", "plugin_id", "名称", "简称", "型号", "版本", "功能描述", "操作", "加入常用"]
        model.setHorizontalHeaderLabels(header)
