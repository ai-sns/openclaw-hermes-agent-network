...
class SparkAI(ModelClient):
    ...
    def create(self, params):
        ...
        if params.get("stream", False) and "messages" in params:
            ...
            # function_definition=[{'name': 'calculator', 'description': '你是一个计算器，可以帮我计算两个数的加减乘除', 'parameters': {'type': 'object', 'properties': {'a': {'type': 'integer'}, 'b': {'type': 'integer'}, 'operator': {'type': 'string'}}, 'required': ['a', 'b', 'operator']}}]
            if 'tools' in params:
                function_definition = params["tools"][0]["function"]

                try:
                    spark_response = self.spark_client.generate([spark_messages], callbacks=[handler], function_definition=[function_definition])
                except Exception as e:
                    raise Exception("spark用户中断")
            ...
            try:
                for generation in spark_response.generations:
                    print("cjr get the chunk:", generation)
