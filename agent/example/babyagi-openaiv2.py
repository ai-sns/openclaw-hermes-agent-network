...
class BabyAGI(Chain,BaseModel):
    ...
    def confirm_tasks(self) -> bool:
        while True:
            user_input = input("是否满意当前任务列表？(yes/no): ").strip().lower()
        ...
        while True:
            if self.task_list:
                while True:
                    ...
                    if self.confirm_tasks():
                        break
                    else:
                        print("重新生成任务列表...")
                        # 可以在这里加入重新生成任务的逻辑，如果需要的话。
                        break  # 之后添加逻辑后，可以去掉这个break
...
if __name__ == "__main__":
    ...
    OBJECTIVE = f"Write a weather report for {city} today"
