...
from PyQt5.QtWidgets import QApplication, QDialog, QMenu, QTableView, QVBoxLayout, QAction, QAbstractItemView, QDialogButtonBox, QMessageBox, QCheckBox, QSlider, QWidget, QHBoxLayout, QLabel
...
class FreezeTableDialog(QDialog):
    ...
    def initUI(self):
        ...
        self.model.setHorizontalHeaderItem(self.model.columnCount() - 1, QStandardItem("滑块"))
        for row in range(self.model.rowCount()):
            slider_widget = SliderWidget()
            self.tableView.setIndexWidget(self.model.index(row, self.model.columnCount() - 1), slider_widget)
    ...
    def accept_close(self):
        self.checkbox_states_and_values.clear()
        for row in range(self.model.rowCount()):
            checkbox_item = self.model.item(row, 0)
            if checkbox_item and checkbox_item.checkState() == Qt.Checked:
                second_column_data = self.model.index(row, 1).data()
                pluginfullname = f"{self.model.index(row, 2).data()}: {self.model.index(row, 4).data()}"
    ...
    def moveSelectedRowsUp(self):
        selected_indexes = self.tableView.selectionModel().selectedRows()
        if selected_indexes:
            rows_to_move = sorted([index.row() for index in selected_indexes])
    ...
    def moveSelectedRowsDown(self):
        selected_indexes = self.tableView.selectionModel().selectedRows()
        if selected_indexes:
            rows_to_move = sorted([index.row() for index in selected_indexes], reverse=True)
            for row in rows_to_move:
