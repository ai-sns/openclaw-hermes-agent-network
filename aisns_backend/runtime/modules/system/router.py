...
async def submit_system_init(
    ...
    try:
        ...
        # Initial position is generated by the remote server as the single
        # source of truth: it picks a base point and scatters it within a 10km
        # radius so freshly registered users do not cluster on the same
        # coordinate, then returns the final coordinate in the response. We only
        # forward an explicit client-provided position if one was supplied;
        # otherwise we let the remote decide and persist whatever it returns.
        register_data = {
            "nation_id": "",
        ...
        if payload.longitude is not None and payload.latitude is not None:
            register_data["longitude"] = payload.longitude
            register_data["latitude"] = payload.latitude
        ...
        # Persist the final position returned by the remote server so the SNS
        # Current Status / Resource panels have a valid current_position
        # immediately, without waiting for the map iframe to initialize it.
        registered_position = None
        try:
            r_lng = remote_res.get("longitude")
            r_lat = remote_res.get("latitude")
            if r_lng is not None and r_lat is not None:
                registered_position = {"lng": float(r_lng), "lat": float(r_lat)}
        except (TypeError, ValueError):
            registered_position = None

        # Fallback for older remote servers that do not return the final
        # coordinate: generate one locally so current_position is still set.
        if registered_position is None:
            from runtime.modules.map.service import MapService
            registered_position = MapService.generate_initial_position()

        service.submit(record, nation_id, current_position=registered_position)
