    ...
    def update_system_config(**kwargs) -> None:
        ...
        if "language" in payload and payload["language"]:
            try:
                ...
                from runtime.i18n import reload_lang
        ...
        if record:
            ...
            try:
                from runtime.apps.sns.service_async import apply_runtime_system_config
...
class SystemInitWizardService:
    ...
    def list_avatar3d(self, request_base_url: str) -> List[Dict[str, str]]:
        ...
        return [
            {
                "key": key,
                "png_url": f"{base}/static/avatar3d/{png_files[key]}",
                "glb_url": f"{base}/static/avatar3d/{glb_files[key]}",
            }
            for key in keys
        ]
