...
def apply_runtime_system_config(payload: dict) -> bool:
    ...
    if "memory_enabled" in payload and _social_engine_instance is not None:
        try:
            from runtime.apps.sns.memory import MemoryConfig
...
class SNSService:
    ...
    async def upload_avatar(self, user_id: str = None, file=None):
        """Upload avatar image"""
        try:
            from runtime.modules.system.service import SystemInitWizardService
    ...
    async def get_resource_overview(self):
        ...
        try:
            ...
            try:
                from runtime.apps.sns.mixin.tools_mixin import ToolsMixin
                from runtime.apps.sns.mixin.data_query_mixin import DataQueryMixin
                ...
                class _RemoteListAdapter(ToolsMixin, DataQueryMixin):
                    def __init__(self, *, base_url: str, position: list, nationid: str):
                        ...
                        self.aisns_cfg_record = _Cfg()
                        self.aisns_cfg_record.current_position = position
            ...
            from runtime.apps.sns.mixin.ui_display_mixin import UIDisplayMixin
    ...
    async def update_map_config(self, data: dict):
        """Update map configuration in aisns_cfg and replace in files"""
        import json
    ...
    async def get_model_info(self):
        ...
        try:
            ...
            if not aisns_cfg or not aisns_cfg.agent_id:
                return {
                    "success": True,
                    "data": {
                        "provider": "N/A",
            ...
            result = await self.db.execute(
                select(AgentCfg).where(AgentCfg.id == aisns_cfg.agent_id)
            )
