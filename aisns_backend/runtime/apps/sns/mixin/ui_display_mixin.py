...
class UIDisplayMixin:
    ...
    def get_on_going_process(self):
        ...
        """
        # Get base info
        profession = self.aisns_cfg_record.profession
        lng = f"{self.aisns_cfg_record.current_position[0]}" if self.aisns_cfg_record.current_position and len(self.aisns_cfg_record.current_position) >= 2 else "0"
        lat = f"{self.aisns_cfg_record.current_position[1]}" if self.aisns_cfg_record.current_position and len(self.aisns_cfg_record.current_position) >= 2 else "0"

        # Build formatted text
        result = "📊 Current Status\n"
        result += "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        result += f"💰 Money      : {self.aisns_cfg_record.money:,.2f}\n"
        result += f"❤️ Life           : {self.aisns_cfg_record.life_point}\n"
        result += f"⚡ Energy      : {self.aisns_cfg_record.energy_point}\n"
    ...
    def get_ai_model_display_name(self):
        ...
        try:
            ...
            # Get account info
            snsaccount = self.aisns_cfg_record.account
    ...
    def update_map_charts(self):
        ...
        radar_data = [
            self.aisns_cfg_record.iq_point,
            self.aisns_cfg_record.energy_point,
            self.aisns_cfg_record.life_point,
            self.aisns_cfg_record.move_point,
            self.aisns_cfg_record.exp_point
        ]
        radar_categories = [
            f'{lt("IQ", "IQ")}:{self.aisns_cfg_record.iq_point}',
            f'{lt("Energy", "Energy")}:{self.aisns_cfg_record.energy_point}',
            f'{lt("Life", "Life")}:{self.aisns_cfg_record.life_point}',
            f'{lt("Move", "Move")}:{self.aisns_cfg_record.move_point}',
            f'{lt("Exp", "Exp")}:{self.aisns_cfg_record.exp_point}'
        ]

        # Prepare bar chart data
        formatted_number = f"{self.aisns_cfg_record.money:,.2f}"
        bar_indicators = [
            f'{lt("Money", "Money")}:{formatted_number}',
            f'{lt("Credit", "Credit")}:{self.aisns_cfg_record.credit}',
            f'{lt("Level", "Level")}{self.aisns_cfg_record.level}'
        ]
        bar_values = [100, self.aisns_cfg_record.credit, self.aisns_cfg_record.level * 10]
        ...
        user_stats = {
            ...
            "level": int(self.aisns_cfg_record.level) if self.aisns_cfg_record.level is not None else 3,
            "credit": int(self.aisns_cfg_record.credit) if self.aisns_cfg_record.credit is not None else 0,
            "money": float(self.aisns_cfg_record.money) if self.aisns_cfg_record.money is not None else 0.0,
            "life": int(self.aisns_cfg_record.life_point) if self.aisns_cfg_record.life_point is not None else 100,
            "iq": int(self.aisns_cfg_record.iq_point) if self.aisns_cfg_record.iq_point is not None else 100,
            "energy": int(self.aisns_cfg_record.energy_point) if self.aisns_cfg_record.energy_point is not None else 100,
            "move": float(self.aisns_cfg_record.move_point) if self.aisns_cfg_record.move_point is not None else 100.0,
            "exp": int(self.aisns_cfg_record.exp_point) if self.aisns_cfg_record.exp_point is not None else 0
        }
