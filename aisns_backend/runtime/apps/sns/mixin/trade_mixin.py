...
from runtime.shared.utils import robust_json_loads, safe_json_dumps
...
class TradeMixin:
    ...
    def _ensure_sufficient_funds(
        ...
        *,
        ...
        try:
            available = float(getattr(getattr(self, "aisns_cfg_record", None), "money", 0) or 0)
        except Exception:
            available = 0.0
    ...
    def get_guidance(self):
        ...
        money_before = float(self.aisns_cfg_record.money or 0)
        self.aisns_cfg_record.money = money_before - fee
        self._show_trade_success_info(
            title="Guidance purchased",
            paid=float(fee),
            money_before=money_before,
            money_after=float(self.aisns_cfg_record.money or 0),
        )
        return result
    ...
    def set_food_order(self):
        ...
        if provider:
            ...
            self.aisns_cfg_record.energy_point = energy_before + 25
            self.aisns_cfg_record.move_point = round(
                100 * (float(self.aisns_cfg_record.life_point or 0) / 100) * (float(self.aisns_cfg_record.energy_point or 0) / 100),
                1,
            )
            self.aisns_cfg_record.money = money_before - fee
            self._show_trade_success_info(
                ...
                energy_after=float(self.aisns_cfg_record.energy_point or 0),
                money_before=money_before,
                money_after=float(self.aisns_cfg_record.money or 0),
            )

        result = f"You paid {fee} for food. Your energy is now {self.aisns_cfg_record.energy_point}%"
        return result
    ...
    def set_taxi_order(self, current_position, target_position, target_place):
        ...
        money_before = float(self.aisns_cfg_record.money or 0)
        self.aisns_cfg_record.money = money_before - float(fee or 0)
        self._show_trade_success_info(
            title="Taxi ordered",
            paid=float(fee or 0),
            money_before=money_before,
        ...
        self.aisns_cfg_record.last_position = current_position
        self.aisns_cfg_record.current_position = target_position
        new_pos = self.aisns_cfg_record.current_position
    ...
    def call_a_doctor(self):
        ...
        if provider:
            ...
            self.aisns_cfg_record.life_point = life_before + 25
            self.aisns_cfg_record.move_point = round(
                100 * (float(self.aisns_cfg_record.life_point or 0) / 100) * (float(self.aisns_cfg_record.energy_point or 0) / 100),
                1,
            )
            ...
            self._show_trade_success_info(
                ...
                life_after=float(self.aisns_cfg_record.life_point or 0),
                money_before=money_before,
                money_after=float(self.aisns_cfg_record.money or 0),
            )

        result = f"You paid {fee} for remote medical service. Your life is now {self.aisns_cfg_record.life_point}%"
        return result
    ...
    def send_pay(
        ...
        if profession_key == "doctor" or (isinstance(good_name, str) and good_name.strip().lower() == "medical"):
            try:
                self.aisns_cfg_record.life_point = float(self.aisns_cfg_record.life_point or 0) + 25
                self.aisns_cfg_record.move_point = round(
                    100 * (float(self.aisns_cfg_record.life_point or 0) / 100) * (float(self.aisns_cfg_record.energy_point or 0) / 100),
                    1,
                )
            ...
            try:
                self.aisns_cfg_record.energy_point = float(self.aisns_cfg_record.energy_point or 0) + 25
                self.aisns_cfg_record.move_point = round(
                    100 * (float(self.aisns_cfg_record.life_point or 0) / 100) * (float(self.aisns_cfg_record.energy_point or 0) / 100),
                    1,
                )
        ...
        try:
            ...
            self._show_trade_success_info(
                ...
                energy_after=float(self.aisns_cfg_record.energy_point or 0) if show_energy else None,
                life_before=life_before if show_life else None,
                life_after=float(self.aisns_cfg_record.life_point or 0) if show_life else None,
                money_before=money_before,
            ...
            current_credit = int(self.aisns_cfg_record.credit or 0)
            self.aisns_cfg_record.credit = current_credit + 1
            trade_type = "B"
    ...
    def handle_send_goods(self, good_str, trade_id):
        ...
        try:
            ...
            trade_type = "S"
            title = (
                (getattr(getattr(self, "aisns_cfg_record", None), "goods_or_service_description", None) or "").strip()
    ...
    def add_money(self, count):
        money = float(self.aisns_cfg_record.money or 0) + count
        self.aisns_cfg_record.money = money
