...
class MapMovementMixin:
    ...
    def _calc_allowed_distance_m(self, base_distance_m: int = 500) -> int:
        try:
            move_point = float(getattr(self.aisns_cfg_record, "move_point", 0) or 0)
        except Exception:
            move_point = 0.0
    ...
    def go_around(self):
        ...
        self.aisns_cfg_record.last_position = self.aisns_cfg_record.current_position
        self.aisns_cfg_record.current_position = [target_position.longitude, target_position.latitude]
    ...
    def move_ahead(self, current_position, target_position, target_place):
        ...
        try:
            ...
            if actual_distance == 0:
                self.aisns_cfg_record.last_position = self.aisns_cfg_record.current_position
                self.aisns_cfg_record.current_position = [current_position.longitude, current_position.latitude]
            ...
            self.aisns_cfg_record.last_position = self.aisns_cfg_record.current_position
            self.aisns_cfg_record.current_position = [next_position.longitude, next_position.latitude]

            new_pos = self.aisns_cfg_record.current_position
            command = ("move_to_a_place", str(new_pos[0]), str(new_pos[1]))
            self.send_msg_to_map(command)

            print("last_position", self.aisns_cfg_record.last_position)
            print("current_position", self.aisns_cfg_record.current_position)
    ...
    def update_after_moving(self):
        ...
        try:
            update_AISnsCfg_map(current_position=json.dumps({"lng": float(lng), "lat": float(lat)}, ensure_ascii=False))
        except Exception as e:
        ...
        try:
            cfg = query_AISnsCfg_map()
            if not cfg:
                logger.warning("Remote location sync skipped: aisns_cfg not found")
                return
