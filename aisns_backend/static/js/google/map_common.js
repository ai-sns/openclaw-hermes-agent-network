...
function initMap() {
    ...
    map.addListener("click", (e) => {
        ...
        const myGeo = (typeof __getMyPersonGeo === 'function') ? __getMyPersonGeo() : null;
        const myPoint = (myGeo && Number.isFinite(Number(myGeo.lat)) && Number.isFinite(Number(myGeo.lng)))
            ? new google.maps.LatLng(Number(myGeo.lat), Number(myGeo.lng))
            : center_point;
        distance = getDistance(myPoint, last_click_point);
            ...
            try {
                ...
                if (typeof setPersonPointByNationId === 'function') {
                    ...
                    if (lngVal !== null && latVal !== null) {
                        ...
                        console.log(distance);
                        if(distance>50000){
                            findHim();//The model will not show correct if move across the map. so must call findHim function，this can refresh
...
function showprofile(nation_id) {
    ...
    if (!person_point && map && typeof map.getCenter === 'function') {
        person_point = map.getCenter();
    }
...
function showprofile3d(geoGroup) {
    ...
    let nation_id = (person && person.nation_id !== undefined && person.nation_id !== null)
        ? String(person.nation_id)
        : '';

    let person_point = nation_id ? getPersonPointByNationId(nation_id) : null;
