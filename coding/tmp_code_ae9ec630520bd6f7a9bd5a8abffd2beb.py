...
def fetch_latest_trump_news():
    ...
    try:
        ...
        for item in soup.find_all('article'):
            # Use more general selectors to capture titles and sources
            title_tag = item.find('h3') or item.find('h2')  # Check for both h3 and h2 tags
            source_tag = item.find('a')  # Look for the first anchor tag for sources
